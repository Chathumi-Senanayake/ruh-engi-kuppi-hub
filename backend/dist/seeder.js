...
const seedDB = async () => {
    try {
        ...
        await Module_1.default.deleteMany({});
        console.log('Cleared existing data');
        const createdDepartments = await Department_1.default.insertMany(departmentsData);
        ...
        const createdModules = await Module_1.default.insertMany(modulesData);
        ...
        const demoUser = await mongoose_1.default.connection.collection('users').insertOne({
            name: 'Master Admin',
            email: 'admin',
        ...
        if (ee1301) {
            await mongoose_1.default.connection.collection('resources').deleteMany({});
            const resourcesData = [
                {
            ...
            await mongoose_1.default.connection.collection('resources').insertMany(resourcesData);
